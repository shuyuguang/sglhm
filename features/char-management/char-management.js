/* char-management.js (Adapted from user-setting.js with independent storage) */
document.addEventListener('DOMContentLoaded', async function() {
    // ====================【Dexie.js 数据库封装】====================
    // 数据库名称 'userSettingsDB' 保持不变，我们只在内部使用不同的 key 来隔离数据
    ...
    // 【修改点】为角色管理定义一个独立的默认ID
    const DEFAULT_CHAR_ID = 'default-char-1';
    let currentProfileId = DEFAULT_CHAR_ID;
    ...
    async function handleConfirmRenameSection() {
        ...
        if (presetContentStore.hasOwnProperty(oldName)) {
            ...
            // 【修改点】预设栏目也需要独立存储
            await dbStorage.setItem('charPresetContentStore', presetContentStore);
        }
        closeNamePrompt();
    }
    ...
    async function loadProfileData(profileId) {
        // 如果数据为空或找不到ID，则加载默认角色
        if (!profileData || profileData.length === 0 || !profileData.find(p => p.id === profileId)) {
            await initializeApp(); // 重新初始化以确保有数据
            return;
        }
        
        const profile = profileData.find(p => p.id === profileId);
        if (!profile) return; // 双重检查

        currentProfileId = profileId;
        // 【修改点】使用新的 key 保存当前选择的角色ID
        await dbStorage.setItem('charCurrentProfileId', currentProfileId);
    ...
    settingsDeleteBtn?.addEventListener('click', async () => {
        ...
        const confirmMessage = `确定要删除 ${selectedProfileIds.length} 个选定的角色吗？\n此操作不可撤销。`;
        if (confirm(confirmMessage)) {
            ...
            if (selectedProfileIds.includes(currentProfileId)) {
                // 【修改点】如果当前角色被删除，则切换回默认角色
                await loadProfileData(DEFAULT_CHAR_ID);
            }
    ...
    createNewUserBtn?.addEventListener('click', async () => {
        const newUser = {
            ...
            avatar: 'https://i.postimg.cc/7hCmXR0s/a-felotus.jpg', banner: 'hhttps://i.postimg.cc/NjRJ5qdx/a-good.jpg'
        };
    ...
    async function initializeApp() {
        // 1. 【修改点】加载角色数据，使用 'charProfileData' 键
        const loadedProfiles = await dbStorage.getItem('charProfileData');
        ...
        // 2. 【修改点】加载角色预设栏目数据，使用 'charPresetContentStore' 键
        const loadedPresets = await dbStorage.getItem('charPresetContentStore');
        if (loadedPresets) {
        ...
        // 3. 【修改点】加载上次选中的角色ID，使用 'charCurrentProfileId' 键
        const loadedProfileId = await dbStorage.getItem('charCurrentProfileId');
        if (loadedProfileId && profileData.some(p => p.id === loadedProfileId)) {
            currentProfileId = loadedProfileId;
        } else {
            currentProfileId = DEFAULT_CHAR_ID; // 如果不存在，则重置为默认角色
            await dbStorage.setItem('charCurrentProfileId', currentProfileId);
        }
